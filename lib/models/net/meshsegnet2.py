...
class STNkd(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        if x.shape[0] == 1:
            x = torch.cat([x, x.clone().detach()])
            x = F.relu(self.bn5(x)[0].unsqueeze(0))
        else:
            x = F.relu(self.bn5(x))
        # x = F.relu(self.bn5(self.fc2(x)))
        x = self.fc2(x)
...
class MeshSegNet(nn.Module):
    def __init__(self, num_classes=15, num_channels=15, with_dropout=True, dropout_p=0.5):
        ...
        self.edge_conv1 = nn.Sequential(nn.Conv2d(30, 32, kernel_size=1, bias=False),
                                        nn.BatchNorm2d(32),
                                        nn.LeakyReLU(negative_slope=0.2))
        self.edge_conv2 = nn.Sequential(nn.Conv2d(30, 128, kernel_size=1, bias=False),
                                        nn.BatchNorm2d(128),
                                        nn.LeakyReLU(negative_slope=0.2))
        ...
        self.glm2_conv3_1 = torch.nn.Conv1d(256, 256, 1)
        self.glm2_bn3_1 = nn.BatchNorm1d(256)
    ...
    def forward(self, x):
        ...
        a_s = get_graph_feature(x, k=6)
        a_l = get_graph_feature(x, k=12)
        # MLP-1
        ...
        # GLM-1
        edge_conv1 = self.edge_conv1(a_s)
        x2_1 = edge_conv1.max(dim=-1, keepdim=False)[0]
        ...
        edge_conv2 = self.edge_conv2(a_s)
        edge_conv3 = self.edge_conv2(a_l)
        x_mlp2 = x_mlp2.transpose(2, 1)
        
        x = F.relu(self.glm2_bn1_1(self.glm2_conv1_1(x_mlp2)))
        x2_2 = edge_conv2.max(dim=-1, keepdim=False)[0]
        x2_3 = edge_conv3.max(dim=-1, keepdim=False)[0]
        
        x2_temp = torch.cat([x2_2, x2_3], dim=1)
        x2_temp = F.relu(self.glm2_bn3_1(self.glm2_conv3_1(x2_temp)))
        
        x = torch.cat([x, x2_temp], dim=1)
